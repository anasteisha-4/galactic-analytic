...
import {
  isAnalyticResults,
  type AnalyticData,
} from '~/utils/is-analytic-results';
...
export const useAnalyticStore = create<AnalyticState>((set) => ({
  ...
  processAnalytics: async (file: File) => {
    ...
    try {
      ...
      let buffer = '';
      let currentData: AnalyticData | null = null;
      ...
      while (!analyticComplete) {
        ...
        if (done) {
          ...
          const newEntry: HistoryEntry = {
            ...
            timestamp: Date.now(),
            status: 'success',
            analyticResults: currentData,
          };
          useHistoryStore.getState().addHistoryEntry(newEntry);
        } else {
          ...
          while (newlineIndex !== -1) {
            ...
            if (jsonLine) {
              const data: AnalyticData = JSON.parse(jsonLine);
              if (isAnalyticResults(data)) {
                set({ analyticResults: data });
                currentData = data;
              }
